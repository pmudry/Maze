...
import maze.display.TextDisplay_BEGIN;
...
public class AStar_BEGIN {
	...
	private AStar_BEGIN(MazeContainer mazeContainer) {
	...
	public static int[][] solve(MazeContainer mc, int x, int y){
		AStar_BEGIN alg = new AStar_BEGIN(mc);
		alg.solve(x, y);
		return alg.solution;
	}
	...
	public static void main(String args[]) {
		...
		TextDisplay_BEGIN.displayMaze(mc);
		
		/**
		 * Compute a solution and display it
		 */
		int[][] solution = AStar_BEGIN.solve(mc, 0, 0);
		AStar_BEGIN.displaySolution(solution);
	}
